...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_removes_rows_with_missing_values(self):
        ...
        '''1.- Crear un DataFrame con valores faltantes usando make_sample_df()'''
        df = make_sample_df()
        '''2.- Llamar a drop_invalid_rows con las columnas "name" y "age"'''
        cleaner = DataCleaner()
        result = cleaner.drop_invalid_rows(df, ["name", "age"])
        '''3.- Verificar que el DataFrame resultante no tiene valores faltantes en esas columnas (usar self.assertEqual para comparar .isna().sum() con 0 - comparación simple de enteros, unittest es suficiente)'''
        self.assertEqual(result["name"].isna().sum(), 0)
        self.assertEqual(result["age"].isna().sum(), 0)
        '''4.- Verificar que el DataFrame resultante tiene menos filas que el original (usar self.assertLess con len() - comparación simple de enteros, unittest es suficiente)'''
    ...
    def test_drop_invalid_rows_raises_keyerror_for_unknown_column(self):
        ...
        """
        '''1.- Crear un DataFrame usando make_sample_df()'''
        df = make_sample_df()
        '''2.- Llamar a drop_invalid_rows con una columna que no existe (ej: "does_not_exist")'''
        '''3.- Verificar que se lanza un KeyError (usar self.assertRaises)'''
        cleaner = DataCleaner()
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        '''1.- Crear un DataFrame con espacios en blanco usando make_sample_df()'''
        df = make_sample_df()
        '''2.- Llamar a trim_strings con la columna "name"'''
        cleaner = DataCleaner()
        result = cleaner.trim_strings(df, ["name"])
        '''3.- Verificar que el DataFrame original no fue modificado (mantiene los espacios) (usar self.assertEqual para comparar valores específicos como strings individuales - unittest es suficiente para strings)'''
        self.assertEqual(df.loc[0, "name"], " Alice ")
        '''4.- Verificar que en el DataFrame resultante los valores de "name" no tienen espacios al inicio/final (usar self.assertEqual para comparar valores específicos como strings individuales - unittest es suficiente)'''
        self.assertEqual(result.loc[0, "name"], "Alice")
        '''5.- Verificar que las columnas no especificadas (ej: "city") permanecen sin cambios (si comparas Series completas, usar pandas.testing.assert_series_equal() ya que maneja mejor los índices y tipos de Pandas; si comparas valores individuales, self.assertEqual es suficiente)'''
    ...
    def test_trim_strings_raises_typeerror_for_non_string_column(self):
        ...
        '''1.- Crear un DataFrame usando make_sample_df()'''
        df = make_sample_df()
        '''2.- Llamar a trim_strings con una columna numérica (ej: "age")'''
        '''3.- Verificar que se lanza un TypeError (usar self.assertRaises)'''
        cleaner = DataCleaner()
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        '''1.- Crear un DataFrame con valores extremos usando make_sample_df()'''
        df = make_sample_df()
        '''2.- Llamar a remove_outliers_iqr con la columna "age" y factor=1.5'''
        cleaner = DataCleaner()
        result = cleaner.remove_outliers_iqr(df, "age", factor=1.5)
        '''3.- Verificar que el valor extremo (120) fue eliminado del resultado'''
        self.assertNotIn(120, result["age"].values)
        '''4.- Verificar que al menos uno de los valores no extremos (25 o 35) permanece en el resultado'''
    ...
    def test_remove_outliers_iqr_raises_keyerror_for_missing_column(self):
        ...
        '''1.- Crear un DataFrame usando make_sample_df()'''
        df = make_sample_df()
        '''2.- Llamar a remove_outliers_iqr con una columna que no existe (ej: "salary")'''
        '''3.- Verificar que se lanza un KeyError (usar self.assertRaises)'''
        cleaner = DataCleaner()
        with self.assertRaises(KeyError):
    ...
    def test_remove_outliers_iqr_raises_typeerror_for_non_numeric_column(self):
        ...
        '''1.- Crear un DataFrame usando make_sample_df()'''
        df = make_sample_df()
        '''2.- Llamar a remove_outliers_iqr con una columna de texto (ej: "city")'''
        '''3.- Verificar que se lanza un TypeError (usar self.assertRaises)'''
        cleaner = DataCleaner()
        with self.assertRaises(TypeError):

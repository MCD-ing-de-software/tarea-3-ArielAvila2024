...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        '''1.- Crear una lista de números (ej: [1, 2, 3, 4])'''
        arr = [1, 2, 3, 4]
        '''2.- Llamar a moving_average con window=2'''
        utils = StatisticsUtils()
        result = utils.moving_average(arr, window=2)
        '''3.- Verificar que el resultado es correcto'''
        expected = np.array([1.5, 2.5, 3.5])
        npt.assert_allclose(result, expected, rtol=1e-7, atol=1e-7)
        '''4.- Verificar que el resultado tiene la forma esperada'''
    ...
    def test_moving_average_raises_for_invalid_window(self):
        ...
        '''1.- Crear una lista de números (ej: [1, 2, 3])'''
        arr = [1, 2, 3]
        '''2.- Llamar a moving_average con window=0 y verificar que se lanza un ValueError'''
        utils = StatisticsUtils()
        with self.assertRaises(ValueError):
            utils.moving_average(arr, window=0)
        '''3.- Llamar a moving_average con window mayor que la longitud del array y verificar que se lanza un ValueError'''
        with self.assertRaises(ValueError):
    ...
    def test_moving_average_only_accepts_1d_sequences(self):
        ...
        """
        '''1.- Crear una secuencia bidimensional (ej: [[1, 2], [3, 4]])'''
        arr = [[1, 2], [3, 4]]
        '''2.- Llamar a moving_average con esa secuencia y verificar que se lanza un ValueError indicando que solo se aceptan secuencias 1D'''
        utils = StatisticsUtils()
        with self.assertRaises(ValueError):
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        """
        '''1.- Crear una lista de números (ej: [10, 20, 30, 40])'''
        arr = [10, 20, 30, 40]
        '''2.- Llamar a zscore para obtener los z-scores (resultado es un array de NumPy)'''
        utils = StatisticsUtils()
        result = utils.zscore(arr)
        '''3.- Verificar que la media del resultado es aproximadamente 0'''
        self.assertAlmostEqual(result.mean(), 0, places=7)
        '''4.- Verificar que la desviación estándar del resultado es aproximadamente 1'''
    ...
    def test_zscore_raises_for_zero_std(self):
        ...
        """
        '''1.- Crear una lista con todos los valores iguales (ej: [5, 5, 5])'''
        arr = [5, 5, 5]
        '''2.- Llamar a zscore con esa secuencia y verificar que se lanza un ValueError'''
        utils = StatisticsUtils()
        with self.assertRaises(ValueError):
            utils.zscore(arr)
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        """
        '''1.- Crear una lista de números (ej: [2, 4, 6])'''
        arr = [2, 4, 6]
        '''2.- Llamar a min_max_scale para obtener los valores escalados (resultado es un array de NumPy)'''
        utils = StatisticsUtils()
        result = utils.min_max_scale(arr)
        '''3.- Verificar que el valor mínimo del resultado es 0.0'''
        self.assertAlmostEqual(result.min(), 0.0, places=7)
        '''4.- Verificar que el valor máximo del resultado es 1.0'''
        self.assertAlmostEqual(result.max(), 1.0, places=7)
        '''5.- Verificar que los valores transformados son correctos'''
    ...
    def test_min_max_scale_raises_for_constant_values(self):
        ...
        """
        '''1.- Crear una lista con todos los valores iguales (ej: [3, 3, 3])'''
        arr = [3, 3, 3]
        '''2.- Llamar a min_max_scale con esa secuencia y verificar que se lanza un ValueError'''
        utils = StatisticsUtils()
        with self.assertRaises(ValueError):
